...
class Bj88Controller extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.pages.bj88', compact("presentCurType","m_count", "collectionKeys", "currencies", 'bo', 'username', 'completedTask', 'platforms'));
    }
    ...
    public function bj88BO(Request $request)
    {
        ...
        $currencyData = $this->currencyService->currencyCollection($request->currency, 'bj88');
        // dd(empty($currencyData['keywords']));
        $target_dates = $this->getPreviousDays();
        $allResults = [];
        // Check if the response has null index and empty keywords
                        ...
                        foreach ($data as $item) {
                            if (isset($item['bo']) && is_array($item['bo'])) {
                                foreach ($item['bo'] as $key => $value) {
                                    ...
                                    $existingRecord = BO::where('affiliate_username', $value['Affiliate Username'])->where('brand', 'bj88')
    ...
    public function Spreedsheet()
    {
        ...
        foreach ($bos as $bo) {
            ...
            $info = $this->currencyService->spreedsheetId($bo->affiliate_username, 'bj88');
        ...
        if ($sp->successful()) {
            ...
            foreach ($filteredData as $fd) {
                if (isset($fd['status']) && $fd['status'] === 200) {
                    $bo = BO::where('affiliate_username', $fd['keyword'])->where('brand', 'bj88')
