...
class BackendController extends Controller
{
    ...
    public function BdtBOFetcher(Request $request)
    {
        ...
        $currencyData = $this->currencyService->currencyCollection($request->currency, 'baji');
        // dd($currencyData);
        // dd($this->getPreviousDays());
        Log::warning('services return [].', ['services' => $currencyData]);
            ...
            foreach ($target_dates as $d) {
                try {
                    ...
                    $response = Http::timeout(3600)->post($this->url, [
                        ...
                        'targetdate' => $d //# "2024/09/05"format 
                    ]);
    ...
    public function Spreedsheet()
    {
        ...
        foreach ($bos as $bo) {
            ...
            $info = $this->currencyService->spreedsheetId($bo->affiliate_username, 'baji');
            // dd($info);
        ...
        if ($sp->successful()) {
            ...
            if (isset($sdata['data']) && is_array($sdata['data'])) {
                $filteredData = array_slice($sdata['data'], 1);

                foreach ($filteredData as $fd) {
                    // Ensure $fd is an array and 'status', 'keyword', and 'target_date' are set
                    if (is_array($fd) && isset($fd['status'], $fd['keyword'], $fd['target_date']) && $fd['status'] === 200) {
                        ...
                        Log::warning('Missing or invalid data in $fd.', ['fd' => $fd]);
                    }
                }

                return response()->json(['result' => $sdata]);
            } else {
                Log::error('Invalid or missing data in response.', ['response' => $sdata]);
                return response()->json(['error' => 'Invalid or missing data in response'], 500);
            }
        } else {
            return response()->json(['error' => 'Failed to fetch FE data'], 500);
        }
