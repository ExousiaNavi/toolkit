...
class Six6SController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.pages.six6s', compact("presentCurType","m_count", "collectionKeys", "currencies", 'bo', 'username', 'completedTask', 'platforms'));
    }
    ...
    public function six6sBO(Request $request)
    {
        ...
        $currencyData = $this->currencyService->currencyCollection($request->currency, 'six6s');
        ...
        if (!is_null($currencyData['index']) || !empty($currencyData['keywords'])) {
            ...
            foreach ($target_dates as $d) {
                try {
                    ...
                    $boaccount = BoAccount::where('brand', 'six6s')->first();
                    // dd($boaccount);
                        ...
                        foreach ($data as $item) {
                            if (isset($item['bo']) && is_array($item['bo'])) {
                                foreach ($item['bo'] as $key => $value) {
                                    ...
                                    $existingRecord = BO::where('affiliate_username', $value['Affiliate Username'])->where('brand', 'six6s')
    ...
    public function Spreedsheet()
    {
        ...
        foreach ($bos as $bo) {
            ...
            // dd($bo);
            $info = $this->currencyService->spreedsheetId($bo->affiliate_username, 'six6s');
        ...
        if ($sp->successful()) {
            ...
            foreach ($filteredData as $fd) {
                if (isset($fd['status']) && $fd['status'] === 200) {
                    $bo = BO::where('affiliate_username', $fd['keyword'])->where('brand', 'six6s')
