    ...
    <div class="w-full lg:ps-44 mt-2">
        <div class="bg-white border w-full p-2">
            ...
            <div class="flex justify-between items-center p-2 font-bold text-slate-600">
                <div>
                    <h1 class="text-xl capitalize">Welcome to WinRs report, {{ Auth::user()->name }}</h1>
                    <span class="text-sm">This is the available currency for automation.</span>
                </div>
                <div>
                    <a id="winrs_automate" class="winrs_automate py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
                        href="#">
                    ...
                    <a id="winrs_automate_report" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
                        href="#">
                        <svg class="shrink-0 size-4 text-white" xmlns="http://www.w3.org/2000/svg" width="24" viewBox="0 0 512 512" height="24" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor">
                            <!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path d="M222.7 32.1c5 16.9-4.6 34.8-21.5 39.8C121.8 95.6 64 169.1 64 256c0 106 86 192 192 192s192-86 192-192c0-86.9-57.8-160.4-137.1-184.1c-16.9-5-26.6-22.9-21.5-39.8s22.9-26.6 39.8-21.5C434.9 42.1 512 140 512 256c0 141.4-114.6 256-256 256S0 397.4 0 256C0 140 77.1 42.1 182.9 10.6c16.9-5 34.8 4.6 39.8 21.5z"/></svg>      
                        Merge to excel
                        ...
                        <div class="flex-1 md:flex-none flex hover:cursor-pointer hover:scale-105 transform transition-transform duration-300">
                            ...
                            <div class="bg-slate-100 rounded-md p-2 flex flex-col">
                                ...
                                @if (in_array($currency->currency, $completedTask))
                                    <a data-c_type="{{ $currency->currency }}" class="winrs_automate_completed py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-green-50 text-green-500 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
                                        >
                                ...
                                @else   
                                    <a data-c_type="{{ $currency->currency }}" class="winrs_automate_debug py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-orange-100 text-orange-600 hover:bg-orange-200 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none"
                                        >
            ...
            <div class="border">
                {{-- {{ $username }} --}}
                @include('admin.pages.tables.winrs_table', ['bos'=>$bo, 'usernames'=>$username, 'platforms'=>$platforms])
            </div>
        </div>
    </div>
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                $('.f_currency').click(function() {
                    ...
                    if (filterQuery != 'all') {
                        $('#winrs_table tbody tr').each(function() {
                ...
                $('#winrs_search').on('input', function() {
                    var searchQuery = $(this).val().toLowerCase();

                    $('#winrs_table tbody tr').filter(function() {
                        $(this).toggle($(this).text().toLowerCase().indexOf(searchQuery) > -1);
                    });
                });
            
                // test for automating btd
                $('.winrs_automate').click(async function() {
                    // Array of currencies
                    let currencies = ['PKR', 'BDT', 'NPR'];
                    ...
                    //         let response = await asyncRequest(`/admin/winrs/bo`, 'POST', { 'currency': currency }, `Connecting to BO and FE platform to fetch data for ${currency}`);
                ...
                $('#winrs_automate_report').click(function(){
                    completedTime = ''
                ...
                $('.winrs_automate_completed').click(function(){
